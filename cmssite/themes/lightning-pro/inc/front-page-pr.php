...
function lightning_front_pr_blocks_customize_register( $wp_customize ) {
	...
	$wp_customize->add_section(
		'lightning_front_pr', array(
			'title'    => lightning_get_prefix_customize_panel() . __( 'Front Page PR Block', 'lightning-pro' ),
			'priority' => 521,
	...
	$wp_customize->add_control(
		'front_pr_display', array(
			'label'       => __( 'Display Front Page PR Block', 'lightning-pro' ),
			'section'     => 'lightning_front_pr',
			'settings'    => 'lightning_theme_options[front_pr_display]',
			'type'        => 'checkbox',
			'priority'    => 1,
			'description' => __( '* If you want to use the more advanced features and set a PR Block anywhere, please Install the WordPress official directory registration plug-in "VK All in One Expansion Unit (Free)" and use the "VK PR Blocks widgets". ', 'lightning-pro' ),
		)
	);
	...
	for ( $i = 1; $i <= 3; ) {
		...
		$wp_customize->add_control(
			'front_pr_title_' . $i,
			array(
				'label'    => __( 'PR Title', 'lightning-pro' ) . ' ' . $i,
				'section'  => 'lightning_front_pr',
		...
		$wp_customize->add_control(
			'front_pr_summary_' . $i,
			array(
				'label'    => __( 'PR Text', 'lightning-pro' ) . ' ' . $i,
				'section'  => 'lightning_front_pr',
		...
		$wp_customize->add_control(
			'front_pr_link_' . $i,
			array(
				'label'    => __( 'PR Link URL', 'lightning-pro' ) . ' ' . $i,
				'section'  => 'lightning_front_pr',
...
function lighting_front_pr_default_array() {
	$front_pr_default = array(
		...
		'title'   => array(
			1 => __( 'For all purposes', 'lightning-pro' ),
			2 => __( 'Powerful features', 'lightning-pro' ),
			3 => __( 'Surprisingly easy', 'lightning-pro' ),
		),
		'summary' => array(
			1 => __( 'Lightning is a simple and customize easy theme. It corresponds by the outstanding versatility to the all-round also in business sites and blogs.', 'lightning-pro' ),
			2 => __( 'By using the plug-in "VK All in One Expansion Unit (free)", you can use the various functions and rich widgets.', 'lightning-pro' ),
			3 => __( 'Lightning is includes to a variety of ideas for making it easier to business site. Please experience the ease of use of the Lightning.', 'lightning-pro' ),
		),
		'link'    => array(
			1 => esc_url( home_url() ),
			2 => esc_url( home_url() ),
