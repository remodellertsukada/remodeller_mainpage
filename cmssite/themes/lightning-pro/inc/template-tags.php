...
function lightning_get_theme_options_default() {
	$theme_options_default = array(
		...
		'top_slide_url_1'               => __( 'https://lightning.nagoya/', 'lightning-pro' ),
		'top_slide_text_title_1'        => __( 'Simple and Customize easy <br>WordPress theme.', 'lightning-pro' ),
		'top_slide_text_caption_1'      => __( '100% GPL Lisence  and adopting the bootstrap', 'lightning-pro' ),
		'top_slide_text_btn_1'          => __( 'READ MORE', 'lightning-pro' ),
		...
		'top_slide_text_title_2'        => __( 'Johnijirou On Snow', 'lightning-pro' ),
		'top_slide_text_caption_2'      => __( 'Growing up everyday', 'lightning-pro' ),
		'top_slide_text_btn_2'          => __( 'READ MORE', 'lightning-pro' ),
...
function lightning_get_the_archive_title() {
	if ( is_category() ) {
		...
		$title = sprintf( __( 'Author: %s', 'lightning-pro' ), '<span class="vcard">' . get_the_author() . '</span>' );
	} elseif ( is_year() ) {
		$title = get_the_date( _x( 'Y', 'yearly archives date format', 'lightning-pro' ) );
	} elseif ( is_month() ) {
		$title = get_the_date( _x( 'F Y', 'monthly archives date format', 'lightning-pro' ) );
	} elseif ( is_day() ) {
		$title = get_the_date( _x( 'F j, Y', 'daily archives date format', 'lightning-pro' ) );
		...
		if ( $postType ) {
			...
			$title = __( 'Archives', 'lightning-pro' );
		}
	}
...
function lightning_the_footerCopyRight() {
	...
	// copyright
	/*------------------*/
	$lightning_footerCopyRight = '<p>' . sprintf( __( 'Copyright &copy; %s All Rights Reserved.', 'lightning-pro' ), get_bloginfo( 'name' ) ) . '</p>';
	echo apply_filters( 'lightning_footerCopyRightCustom', $lightning_footerCopyRight );

	// Powered
	/*------------------*/
	$lightning_footerPowered = __( '<p>Powered by <a href="https://wordpress.org/">WordPress</a> &amp; <a href="https://lightning.nagoya" target="_blank" title="Free WordPress Theme Lightning"> Lightning Theme</a> by Vektor,Inc. technology.</p>', 'lightning-pro' );
