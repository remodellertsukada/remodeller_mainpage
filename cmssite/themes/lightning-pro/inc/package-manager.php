...
function lightning_old_packages_array() {
	$packages = array(
		'widget_full_wide_title'     => array(
			'label'       => __( 'Full Wide Title Widget', 'lightning-pro' ),
			'description' => __( 'If you are using Lightning Pro that, You can use the same function by Outer Block and Title Block in Plugin VK Blocks.', 'lightning-pro' ),
			'path'        => get_parent_theme_file_path( '/inc/widgets/widget-full-wide-title.php' ),
		),
		'widget_contents_area_posts' => array(
			'label'       => __( 'Content Area Posts Widget', 'lightning-pro' ),
			'description' => __( 'If you are using Lightning Pro that, You can use the more powerful function by Media Posts BS4 Widget and Latest Posts Block in Plugin VK Blocks.', 'lightning-pro' ),
			'path'        => get_parent_theme_file_path( '/inc/widgets/widget-new-posts.php' ),
		),
		'widget_front_pr'            => array(
			'label'       => __( 'Front Page PR Block', 'lightning-pro' ),
			'description' => __( 'You can use same function by PR Block in Plugin VK Blocks', 'lightning-pro' ),
...
function lightning_customize_register_package( $wp_customize ) {
	...
	$wp_customize->add_control(
		new Custom_Html_Control(
			$wp_customize, 'not_recommended_title', array(
				'label'            => __( 'Not recommended functions', 'lightning-pro' ),
				'section'          => 'lightning_function',
				'type'             => 'text',
				'custom_title_sub' => '',
				'custom_html'      => '<p>' . __( 'These are old functions that already alternated to the new function.', 'lightning-pro' ) . '</p>',
			)
		)
	);
	...
	foreach ( $functions as $key => $value ) {
		...
		$wp_customize->add_control(
			"lightning_theme_options[disable_functions][$key]", array(
				'label'       => sprintf( __( 'Stop %s', 'lightning-pro' ), $value['label'] ),
				'section'     => 'lightning_function',
