...
class WP_Widget_ltg_post_list extends WP_Widget {
	...
	function __construct() {
		...
		$widget_name = lightning_get_prefix() . __( 'Content Area Posts Widget', 'lightning-pro' );

		parent::__construct(
			'ltg_post_list',
			$widget_name,
			array( 'description' => __( 'Displays a list of your most recent posts.', 'lightning-pro' ) )
		);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['label'] ) ) {
			...
			echo $args['before_title'];
			echo __( 'Recent Posts', 'lightning-pro' );
			echo $args['after_title'];
		}
	...
	function display_pattern_1() {
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			...
			<div class="<?php lightning_the_class_name( 'entry-body' ); ?>">
				...
				<?php
				$lightning_more_btn_txt = '<span class="btn btn-default btn-block">' . __( 'Read more', 'lightning-pro' ) . '</span>';
			...
			<div class="entry-footer">
				...
				if ( $tags_list ) :
					...
					<div class="entry-meta-dataList entry-tag">
						<dl>
							<dt><?php _e( 'Tags', 'lightning-pro' ); ?></dt>
	...
	function form( $instance ) {
		$defaults = array(
			'count'     => 10,
			'label'     => __( 'Recent Posts', 'lightning-pro' ),
			'post_type' => 'post',
			'terms'     => '',
			'format'    => '0',
			'more_url'  => '',
		...
		<ul>
			<li><label><input type="radio" name="<?php echo $this->get_field_name( 'format' ); ?>" value="0"
						...
						if ( $instance['format'] == 0 ) {
							echo 'checked'; }
						?>
					/><?php echo __( 'Thumbnail', 'lightning-pro' ) . '/' . __( 'Date', 'lightning-pro' ) . '/' . __( 'Category', 'lightning-pro' ) . '/' . __( 'Title', 'lightning-pro' ) . '/' . __( 'Excerpt', 'lightning-pro' ); ?></label>
			</li>
			<li><label><input type="radio" name="<?php echo $this->get_field_name( 'format' ); ?>" value="1"
						...
						if ( $instance['format'] == 1 ) {
							echo 'checked'; }
						?>
					/><?php echo __( 'Date', 'lightning-pro' ) . '/' . __( 'Category', 'lightning-pro' ) . '/' . __( 'Title', 'lightning-pro' ) . '/' . __( 'Content', 'lightning-pro' ); ?></label>
			</li>
		</ul>
		<br/>
		<?php // Title ?>
		<label for="<?php echo $this->get_field_id( 'label' ); ?>"><?php _e( 'Title:', 'lightning-pro' ); ?></label><br/>
		...
		<?php // Post Type ?>
		<label for="<?php echo $this->get_field_id( 'post_type' ); ?>"><?php _e( 'Slug for the custom type you want to display', 'lightning-pro' ); ?>:</label><br />
		...
		<?php // Terms ?>
		<label for="<?php echo $this->get_field_id( 'terms' ); ?>"><?php _e( 'taxonomy ID', 'lightning-pro' ); ?>:</label><br />
		...
		<?php
		_e( 'if you need filtering by term, add the term ID separate by ",".', 'lightning-pro' );
		echo '<br/>';
		...
		<?php // Read more ?>
		<label for="<?php echo $this->get_field_id( 'more_url' ); ?>"><?php _e( 'Destination URL:', 'lightning-pro' ); ?></label><br/>
		...
		<br /><br />
		<label for="<?php echo $this->get_field_id( 'more_text' ); ?>"><?php _e( 'Notation text:', 'lightning-pro' ); ?></label><br/>
		<input type="text" placeholder="<?php _e( 'Latest post list', 'lightning-pro' ); ?>" id="<?php echo $this->get_field_id( 'more_text' ); ?>" name="<?php echo $this->get_field_name( 'more_text' ); ?>" value="<?php echo esc_attr( $instance['more_text'] ); ?>" />
		<br /><br />
		<?php
	}
