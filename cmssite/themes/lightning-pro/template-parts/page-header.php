...
if ( is_search() ) {
	if ( ! empty( get_search_query() ) ) {
		$search_text = sprintf( __( 'Search Results for : %s', 'lightning-pro' ), get_search_query() );
	} else {
		$search_text = __( 'Search Results', 'lightning-pro' );
	}
	...
	$pageTitle  = sprintf( __( 'Search Results for : %s', 'lightning-pro' ), $bbp_search );
} elseif ( is_404() ) {
	$pageTitle = __( 'Not found', 'lightning-pro' );
