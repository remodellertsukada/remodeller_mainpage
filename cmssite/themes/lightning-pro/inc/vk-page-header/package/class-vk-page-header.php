...
if ( ! class_exists( 'Vk_Page_Header' ) ) {
	...
	class Vk_Page_Header {
		...
		public function customize_register( $wp_customize ) {
			...
			$wp_customize->add_section(
				'vk_page_header_setting', array(
					'title'    => $customize_setting_prefix . __( 'Page Header Setting', 'lightning-pro' ),
			...
			$wp_customize->add_control(
				new WP_Customize_Color_Control(
					$wp_customize, 'cover_color', array(
						'label'    => __( 'Cover color', 'lightning-pro' ),
						'section'  => 'vk_page_header_setting',
			...
			$wp_customize->add_control(
				new Vk_Page_Header_Custom_Text_Control(
					$wp_customize, 'cover_opacity', array(
						'label'       	=> __( 'Cover opacity', 'lightning-pro' ),
						'section'     	=> 'vk_page_header_setting',
						...
						'num_min'		=> 0,
						'num_max'		=> 1,
						'description' 	=> __( 'Please enter a number from 0 to 1', 'lightning-pro' ),
					)
				)
			);
			...
			$wp_customize->add_control(
				new WP_Customize_Color_Control(
					$wp_customize, 'text_shadow_color', array(
						'label'    => __( 'Text shadow color', 'lightning-pro' ),
						'section'  => 'vk_page_header_setting',
			...
			$wp_customize->add_control(
				new Vk_Page_Header_Custom_Text_Control(
					$wp_customize, 'text_margin', array(
						'label'       => __( 'Text margin (top and bottom)', 'lightning-pro' ),
						'section'     => 'vk_page_header_setting',
			...
			$wp_customize->add_control(
				new Custom_Html_Control(
					...
					'vk_page_header_title_common',
					array(
						'label'            => __( 'Page header bg image', 'lightning-pro' ) . ' [ ' . __( 'Basic', 'lightning-pro' ) . ' ]',
						'section'          => 'vk_page_header_setting',
						'type'             => 'text',
						'custom_title_sub' => '',
						'custom_html'      => '<p>' . __( 'You can set the original image in the background of the page header part.', 'lightning-pro' ) . '</p>',
					)
				)
			);
			...
			foreach ( $custom_types as $name => $label ) {
				...
				if ( $name == 'page' ) {
					$description = __( 'If you want to change the image of a specific page, you can set it from the editing screen of each fixed page.', 'lightning-pro' ) . '<br>';
				} else {
					$description = '';
				}
				$description .= __( 'When not set, the image of [ Basic ] is applied.', 'lightning-pro' );
				...
				$wp_customize->add_control(
					new Custom_Html_Control(
						...
						'vk_page_header_title_' . $label,
						array(
							'label'            => __( 'Page header bg image', 'lightning-pro' ) . ' [ ' . $label . ' ]',
							'section'          => 'vk_page_header_setting',
							'type'             => 'text',
							'custom_title_sub' => '',
		...
		public function add_pagehead_setting_meta_box() {
			...
			add_meta_box( 'vk_page_header_meta_box', __( 'Page Header Image', 'lightning-pro' ), array( $this, 'vk_page_header_meta_box_content' ), 'page', 'normal', 'high' );
		}
		...
		public static function custom_fields_array() {
			...
			$custom_fields_array = array(
				'vk_page_header_image' => array(
					'label'       => __( 'Page header bg image', 'lightning-pro' ),
					'type'        => 'image',
					'description' => '',
					'required'    => false,
				),
				'vk_page_header_image_sp' => array(
					'label'       => __( 'Page header bg image', 'lightning-pro' ) . ' ( ' . __( 'Mobile', 'lightning-pro' ) . ' )',
					'type'        => 'image',
					'description' => '',
					'required'    => false,
				),
